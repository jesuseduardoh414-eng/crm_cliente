// Datos de muestra de TODO el sistema, con una historia coherente de ferretería
// con renta de maquinaria: equipo por áreas, obras con sus tareas, maquinaria
...
// ── El equipo ──────────────────────────────────────────────────────────────
const EQUIPO = [
  { nombre: 'Laura Medina',    email: `laura${DOMINIO_DEMO}`,    area: 'VENTAS',         rol: 'ADMIN',    puesto: 'Gerente de ventas' },
  { nombre: 'Ricardo Salas',   email: `ricardo${DOMINIO_DEMO}`,  area: 'ALMACEN',        rol: 'MIEMBRO',  puesto: 'Jefe de almacén' },
  { nombre: 'Paty Cruz',       email: `paty${DOMINIO_DEMO}`,     area: 'COMPRAS',        rol: 'MIEMBRO',  puesto: 'Compradora' },
  { nombre: 'Sofía Ibarra',    email: `sofia${DOMINIO_DEMO}`,    area: 'ADMINISTRACION', rol: 'MIEMBRO',  puesto: 'Administración' },
  { nombre: 'Miguel Ángel Ruiz', email: `miguel${DOMINIO_DEMO}`, area: 'RENTA',          rol: 'MIEMBRO',  puesto: 'Coordinador de renta' },
  { nombre: 'Beto Fuentes',    email: `beto${DOMINIO_DEMO}`,     area: 'TALLER',         rol: 'MIEMBRO',  puesto: 'Mecánico' },
  // Operadores de maquinaria
  { nombre: 'Juan Herrera',    email: `juan${DOMINIO_DEMO}`,     area: 'RENTA', rol: 'OPERADOR', puesto: 'Operador',
    perfil: { especialidad: 'Retroexcavadora', zona: 'Zona norte', tarifaHora: 380, experienciaAnios: 12, telefonoContacto: '55 1234 5678', disponible: true,
              descripcion: 'Certificado en retro y minicargador. Disponible entre semana.' } },
  { nombre: 'Luis Ramírez',    email: `luis${DOMINIO_DEMO}`,     area: 'RENTA', rol: 'OPERADOR', puesto: 'Operador',
    perfil: { especialidad: 'Grúa titán', zona: 'Zona sur', tarifaHora: 450, experienciaAnios: 8, telefonoContacto: '55 8765 4321', disponible: true,
              descripcion: 'Maniobras de izaje y montaje de estructura.' } },
  { nombre: 'Chuy Ordóñez',    email: `chuy${DOMINIO_DEMO}`,     area: 'RENTA', rol: 'OPERADOR', puesto: 'Operador',
    perfil: { especialidad: 'Compactadora / bailarina', zona: 'Zona centro', tarifaHora: 260, experienciaAnios: 5, telefonoContacto: '55 2468 1357', disponible: false,
...
const PUBLICACIONES = [
  ...
  { titulo: 'Juan Herrera libre a partir del lunes', tipo: 'OPERADOR_DISPONIBLE', autor: 'miguel',
    cuerpo: 'Termina la excavación en Reforma 120. Certificado en retro y minicargador, zona norte.' },
  { titulo: 'Luis Ramírez disponible para maniobras de izaje', tipo: 'OPERADOR_DISPONIBLE', autor: 'laura',
...
async function limpiar() {
  ...
  await prisma.asignacionMaquina.deleteMany({ where: { OR: [{ proyectoId: { in: pids } }, { operadorId: { in: ids } }] } });
  ...
  await prisma.perfilOperador.deleteMany({ where: { usuarioId: { in: ids } } });
...
async function sembrar() {
  const admin = await prisma.usuario.findFirst({ where: { rol: 'ADMIN', email: { not: { endsWith: DOMINIO_DEMO } } } });
  if (!admin) throw new Error('No hay ADMIN. Corre antes: npm run seed');
  ...
  for (const u of EQUIPO) {
    const creado = await prisma.usuario.create({
      data: {
        ...
        telefono: u.perfil?.telefonoContacto || null,
      },
    });
    porEmail[u.email.split('@')[0]] = creado;

    if (u.perfil) {
      await prisma.perfilOperador.create({ data: { usuarioId: creado.id, ...u.perfil } });
    }
  ...
  await prisma.diaEspecial.create({ data: { usuarioId: porEmail.chuy.id, fecha: dias(3), tipo: 'vacacion', descripcion: 'Vacaciones' } });
  ...
  const ASIGNACIONES = [
    { proyecto: 'Edificio Reforma 120', maquina: 'Retroexcavadora CAT 420F2', operador: 'juan', inicio: -10, fin: 24, notas: 'Excavación de cimientos y nivelación.' },
    { proyecto: 'Edificio Reforma 120', maquina: 'Grúa titán 20 ton', operador: 'luis', inicio: -2, fin: 30, notas: 'Izaje de trabes. Requiere permiso de vía.' },
    ...
    { proyecto: 'Casa habitación Los Encinos', maquina: 'Compactadora Wacker WP1550', operador: 'chuy', inicio: -3, fin: 12, notas: 'Compactado de terracería.' },
  ...
  for (const a of ASIGNACIONES) {
    await prisma.asignacionMaquina.create({
      data: {
        ...
        operadorId: a.operador ? porEmail[a.operador].id : null,
  ...
  console.log('  operadores ...........', await c('perfilOperador'));
  console.log('  maquinas .............', await c('maquina'));
  console.log('  proyectos ............', await c('proyecto'));
  console.log('  tareas ...............', await c('tarea'));
  ...
  console.log(`  Ej: laura${DOMINIO_DEMO} (ADMIN) · juan${DOMINIO_DEMO} (OPERADOR)`);
